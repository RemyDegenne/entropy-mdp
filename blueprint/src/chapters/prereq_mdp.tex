...
(namespace \texttt{Learning.MDP.Episodic}, files \texttt{StepLaw.lean}, \texttt{Occupancy.lean},
\texttt{StateSeq.lean}, \texttt{Unroll.lean}, \texttt{EntropicVariance.lean} and \texttt{Visits.lean} of
...
\end{equation}
The trajectory law $\Prob^\pi_{(s, h)}$ is defined at every step $h \in \N$, and $R^\pi_h = 0$
for $h \ge H + 1$ (the return of $H + 1 - h = 0$ rounds).
...
and the $0$-indexed steps \texttt{h k : ℕ}:
\texttt{hasCondDistrib\_shiftRounds\_stepLaw} states that the trajectory shifted by $k$ rounds
(\texttt{shiftRounds k}) has conditional law \texttt{stepLaw M π (s', h + k)} given the first $k$ rounds
...
with \texttt{lintegral\_stepLaw\_succ}) and converted with
Lemma~\ref{lem:pmdp_hascond_integral}. Almost sure properties of the future given the past are
transported by \texttt{ae\_stepLaw\_of\_ae\_shiftRounds}; the step of the round $k$ is $h + k$
(\texttt{ae\_obs\_eq\_stepLaw}) and its action is $\pi_{h+k}$ of its state
(\texttt{ae\_action\_eq\_stepLaw}, which needs measurable equality on the action space,
\texttt{MeasurableEq A}, e.g.\ a countable discrete action space). The first consequence is
...
\lean{Learning.MDP.Episodic.integral_prod_mul_le_of_backward, Learning.MDP.Episodic.le_sum_integral_of_backward, Learning.MDP.Episodic.le_sum_integral_of_backward_exp}\leanok
\uses{def:episodic_mdp, def:policy, def:step_law}
...
\texttt{integral\_prod\_mul\_le\_of\_backward} is \eqref{eq:pmdp_unroll_ih} for the $0$-indexed
step \texttt{h : ℕ} with \texttt{h ≤ H}, for general multiplicative nonnegative weights
\texttt{w i (S\_i) (A\_i)} (the weight $\prod_{i < h}$ is over \texttt{range h} and the right-hand
side over \texttt{h' ∈ Ico h H}, with the products $\prod_{i \le h'}$ over \texttt{range (h' + 1)}); the
hypotheses are \texttt{0 ≤ κ}, \texttt{0 ≤ w}, \texttt{g H = 0} and the backward inequality with
\texttt{∫ s', g (h + 1) s' ∂M.trans h (s, π h s)}. It is proved by downward induction on
\texttt{h}. The ``in particular'' is \texttt{le\_sum\_integral\_of\_backward}, and with the
exponential weights \texttt{w i s a = exp (β * r i s a)} it is
\texttt{le\_sum\_integral\_of\_backward\_exp}:
\texttt{g 0 s₁ ≤ ∑ h ∈ range H, κ \^{} h * ∫ x, exp (β * ∑ i ∈ range (h + 1), r i (S\_i) (A\_i)) * u h (S\_h) (A\_h) ∂stepLaw M π (s₁, 0)}.
\end{remark}
...
\textbf{Lean remark.} \texttt{entropicVarQ M H β π h s a} is the nested integral
\texttt{∫ x, ∫ s', ∫ traj, (exp (β * (x + episodeReturn (H - (h + 1)) traj)) - expQ M H β π h s a) \^{} 2 ∂stepLaw M π (s', h + 1) ∂M.trans h (s, a) ∂M.reward h (s, a)}
(the mean square deviation from $U^\pi_h(s,a)$ under $\Prob^\pi_{(s,a,h)}$, without naming that
measure), and \texttt{entropicVarV M H β π h s := entropicVarQ M H β π h s (π h s)} for $h < H$ and
$0$ from the step $H$ on (\texttt{entropicVarV\_of\_lt}, \texttt{entropicVarV\_of\_le}). This matches the
...
\lean{Learning.MDP.Episodic.entropicVarV_of_lt, Learning.MDP.Episodic.entropicVarV_of_le, Learning.MDP.Episodic.integrable_exp_episodeReturn_sub_sq, Learning.MDP.Episodic.entropicVarV_eq_integral, Learning.MDP.Episodic.entropicVarV_eq_variance, Learning.MDP.Episodic.entropicVarV_mem_Icc, Learning.MDP.Episodic.measurable_entropicVarV, Learning.MDP.Episodic.integrable_entropicVarV}\leanok
\uses{def:entropic_variance, def:exp_value}
Assume that the rewards are bounded (supported in a fixed interval $[a, b]$; in particular
$e^{2\beta R}$ is integrable). For $1 \le h \le H$ and $s \in \Ss$,
\[
...
\lean{Learning.MDP.Episodic.entropicVarQ_eq_of_hasRewardFn, Learning.MDP.Episodic.entropicVarQ_eq_of_hasRewardFn_vec, Learning.MDP.Episodic.variance_eq_vecVar_transVec}\leanok
...
\lean{Learning.MDP.Episodic.sum_integral_exp_mul_variance_eq_entropicVarV, Learning.MDP.Episodic.sum_integral_exp_mul_variance_eq_variance, Learning.MDP.Episodic.sum_integral_exp_mul_vecVar_eq_variance}\leanok
\uses{def:entropic_variance, def:exp_value, def:reward_fn}
...
\textbf{Lean remark.} With $0$-indexed steps \texttt{h ∈ range H}, the weight of the step
\texttt{h} is \texttt{exp (2 * β * ∑ i ∈ range (h + 1), r i (S\_i) (A\_i))} and the variance term is
\texttt{variance (expValue M H β π (h + 1)) (M.trans h (S\_h, A\_h))} (on a finite state space,
\texttt{vecVar (M.transVec h (S\_h) (A\_h)) (expValue M H β π (h + 1))}), under
...
statements are as follows, for a run \texttt{hseq : IsAlgEnvSeq O X Y alg (statesEnv M H s₁) P}, the
