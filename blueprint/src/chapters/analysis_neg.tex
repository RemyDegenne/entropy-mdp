...
so the lemmas of this chapter are about the \texttt{β < 0} branches of the same declarations; their
names end with \texttt{\_of\_neg} (\texttt{EV2026/AnalysisNeg.lean}). The
formulation at a fixed history under the concentration inequalities
(Definition~\ref{def:pos_hist_concentration}, Lemma~\ref{lem:pos_hist_concentration}) followed by a
run version on \texttt{goodEvent}, the sign-independent cores of Section~\ref{sec:pos_cores}, the
standing hypotheses ($\delta \in (0, 1]$, $\eps \ge 0$), the induction on the Lean step and the
treatment of the never-visited pairs are as in Chapter~\ref{chap:analysis_pos}. The clips of a never-visited pair
...
\lean{Essakine2026Tight.abs_vecExp_sub_le_bonus_add_of_neg, Essakine2026Tight.abs_vecExp_sub_le_bonusAt_add_of_neg}\leanok
...
\lean{Essakine2026Tight.optExpValue_mem_Icc_optZ_of_neg, Essakine2026Tight.optExpQ_mem_Icc_stepUAt_of_neg, Essakine2026Tight.optExpQ_le_stepUAt_fst_of_neg, Essakine2026Tight.stepUAt_snd_le_optExpQ_of_neg, Essakine2026Tight.optExpValue_mem_Icc_ZlowerAt_ZtildeAt_of_neg, Essakine2026Tight.optExpQ_mem_Icc_UlowerAt_UtildeAt_of_neg, Essakine2026Tight.optExpValue_mem_Icc_of_neg, Essakine2026Tight.optZ_mem_of_neg}\leanok
...
nonnegative, and the Lean proof has the same structure with LML's \texttt{Function.min}
(\texttt{Function.min\_le}, \texttt{argmin\_spec}) in place of \texttt{Function.max}. In the Lean proof of
Lemma~\ref{lem:concentration_neg}, the case $h = H$ ($B_H = 0$) needs no separate treatment: the core
Lemma~\ref{lem:pos_core_concentration} is applied with $c = \ell_{h+1}$ and $B = B_h \ge 0$, and
Lemma~\ref{lem:pos_vec_kl} allows the range $0$.
...
\lean{Essakine2026Tight.ringZ_le_one_of_neg, Essakine2026Tight.ringU_le_one_of_neg, Essakine2026Tight.ringUAux_le_one_of_neg, Essakine2026Tight.ringZAt_le_one_of_neg, Essakine2026Tight.ringUAt_le_one_of_neg}\leanok
...
\lean{Essakine2026Tight.optZ_fst_le_ringZ_and_expValue_le_of_neg, Essakine2026Tight.stepUAt_fst_le_ringU_and_expQ_le_of_neg, Essakine2026Tight.stepUAt_fst_le_ringU_of_neg, Essakine2026Tight.expQ_le_ringU_of_neg, Essakine2026Tight.ZtildeAt_le_ringZAt_and_expValue_le_of_neg, Essakine2026Tight.UtildeAt_le_ringUAt_and_expQ_le_of_neg}\leanok
...
\lean{Essakine2026Tight.ringU_sub_stepUAt_snd_le_of_neg, Essakine2026Tight.ringUAt_sub_UlowerAt_le_of_neg}\leanok
...
\begin{proof}
\uses{lem:concentration_neg, lem:optimism_neg, lem:ring_neg, lem:neg_ring_range, lem:kl_bernstein, lem:kl_transport_var, lem:var_le_range_mean, lem:sqrt_add_le, lem:sqrt_mul_le, lem:emp_trans_simplex, lem:backups_range, lem:opt_exp_value_range, def:event_kl, def:bonus, lem:pos_core_third_term}
\leanok
...
\lean{Essakine2026Tight.ringZ_sub_optZ_snd_le_cert_of_neg, Essakine2026Tight.ringZAt_sub_ZlowerAt_le_certAt_of_neg}\leanok
...
\lean{Essakine2026Tight.expValue_sub_optExpValue_le_cert_of_neg, Essakine2026Tight.expValue_sub_optExpValue_le_certAt_of_neg}\leanok
...
\lean{Essakine2026Tight.optEntropicValue_sub_entropicValue_le_of_stopCond_of_neg, Essakine2026Tight.optEntropicValue_sub_entropicValue_greedyAt_le_of_neg}\leanok
...
\lean{Essakine2026Tight.cert_le_of_neg, Essakine2026Tight.certAt_le_of_neg, Essakine2026Tight.expValue_mem_Icc_of_neg, Essakine2026Tight.exp_mul_le_one_of_neg}\leanok
...
\textbf{Lean remark.} The Lean statement (\texttt{cert\_le\_of\_neg}, \texttt{certAt\_le\_of\_neg}) is the
displayed form of Remark~\ref{rem:neg_additive_term}, with the additive term $84H\rho^t_h(s,a)$
outside the factor $e^{\beta r_h(s,a)}$. Step (i) is Lemma~\ref{lem:pos_core_recursion}(i) with the
range $1$, step (ii) is Lemma~\ref{lem:pos_core_recursion}(ii) with $G = \Zl^t_{h+1}$, $Z = Z^\pi_{h+1}$,
$c = \ell_{h+1}$ and $B = B_h$, and step (iii) is Lemma~\ref{lem:pos_core_recursion}(iii) with $B = B_h$,
$B' = C = 1$; the factor $e^{\beta r_h(s,a)} \le 1$ is then dropped from the additive term only.

\begin{lemma}[Unrolled certificate]\label{lem:cert_unrolled_neg}
\lean{Essakine2026Tight.cert_le_exp_mul_sum_integral_of_neg, Essakine2026Tight.certAt_le_exp_mul_sum_integral_of_neg}\leanok
...
\begin{proof}
\uses{lem:cert_true_recursion_neg, lem:unroll_recursion, lem:one_add_div_pow_le_exp, def:certificate, lem:pos_core_unroll}
\leanok
...
\textbf{Lean remark.} The unrolling is Lemma~\ref{lem:pos_core_unroll}(i) with $c = 13$ and the $u_h$ above;
the last simplification is an \texttt{integral\_mono} with the pointwise bound
$e^{\beta\sum_{i \le h}r_i}e^{-\beta r_h} \le 1$ (as $r_h(S_h, A_h) \le \sum_{i \le h}r_i(S_i, A_i)$). The Lean
integrand is $e^{\beta\sum_{i \le h}r_i}(36\sqrt{\cdot}) + 84H\rho^t_h(S_h, A_h)$.

\begin{lemma}[Normalized certificate bound]\label{lem:ratio_bound_neg}
\lean{Essakine2026Tight.cert_div_expValue_le_of_neg, Essakine2026Tight.cert_div_optZ_snd_add_cert_le_of_neg, Essakine2026Tight.certAt_div_ZlowerAt_add_certAt_le_of_neg}\leanok
...
\begin{proof}
\uses{lem:cert_unrolled_neg, lem:ring_neg, lem:cert_dominates_gap_neg, lem:backups_range, lem:exp_value_range, lem:cauchy_schwarz_traj, lem:entropic_variance_sum, lem:variance_ratio_le, lem:gmax_le_horizon, lem:occupancy_expectation, def:entropic_variance, lem:pos_core_unroll}
\leanok
...
use the computable one. In Lean: \texttt{cert\_div\_expValue\_le\_of\_neg} is the second inequality
(with $e^{|\beta|H}$); \texttt{cert\_div\_optZ\_snd\_add\_cert\_le\_of\_neg} combines the first inequality with the
``in particular'' form ($e^{|\beta|(H+1)}$), which is the form of the hypothesis of
Lemma~\ref{lem:pos_core_stopping_time}; its run version \texttt{certAt\_div\_ZlowerAt\_add\_certAt\_le\_of\_neg}
is stated only for this computable ratio. The variance term is Lemma~\ref{lem:pos_core_unroll}(ii).
...
\lean{Essakine2026Tight.progress_le_cert_div_optZ_snd_add_cert_of_neg, Essakine2026Tight.progress_le_certAt_div_ZlowerAt_add_certAt_of_neg}\leanok
...
\textbf{Lean remark.} The Lean proof avoids the monotonicity of $x \mapsto x/(c + x)$: with $v := e^{\beta\eps} \in (0, 1]$,
$\kappa(\beta, \eps) = (1 - v)v/2$, so $\kappa c \le (1 - v)c/2 \le g/2$ and $\kappa g \le g/2$, i.e.\ $\kappa(c + g) \le g$.
No event is assumed; $\eps \ge 0$ and $\delta \in (0, 1]$ suffice.

\begin{lemma}[Bound on the stopping time]\label{lem:stopping_time_bound_neg}
\lean{Essakine2026Tight.stoppingTime_le_upperBound_of_neg}\leanok
...
($X_t(\omega) = \pi^{t+1}$ for all $t$, almost surely by Lemma~\ref{lem:entropic_bpi_run}), the stopping time of
the run satisfies
...
\begin{proof}
\uses{lem:progress_neg, lem:ratio_bound_neg, lem:sum_counts_bound, lem:entropic_bpi_run, lem:self_bounding, def:rates, def:upper_bound_const, lem:rates_props, def:rate_min, lem:pos_core_stopping_time}
\leanok
...
\textbf{Lean remark.} As for Lemma~\ref{lem:stopping_time_bound_pos}: Steps~1--4 are
Lemma~\ref{lem:pos_core_stopping_time}, and the hypothesis $X_t(\omega) = \pi^{t+1}$ is needed to apply the
counting bound (about the policies played) to the ratio bound (about the greedy policies).
...
\lean{Essakine2026Tight.measureReal_bad_le_of_neg, Essakine2026Tight.one_sub_le_measureReal_stoppingTime_le_of_neg}\leanok
...
\begin{proof}
\uses{lem:stopping_time_bound_neg, lem:entropic_bpi_run, lem:pac_at_stopping_neg, lem:good_event, lem:pos_core_pac}
\leanok
...
same Lean statement with the hypotheses \texttt{0 < β} and \texttt{β < 0}. Both are instances of
Lemma~\ref{lem:pos_core_pac} with $E = \evGood^-$.
