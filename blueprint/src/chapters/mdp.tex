...
\uses{def:mdp}
Let $M$ be an MDP with countable state space (discrete $\sigma$-algebra), $\pi$ a measurable
...
countable state space). The trajectory law of $\pi$ as a kernel from the initial state is
\texttt{policyKernel M hπ}; the first round and the state of round $1$ are
...
\lean{Learning.MDP.Episodic.hasLaw_stepLaw_castSucc, Learning.MDP.Episodic.ae_obs_zero_stepLaw, Learning.MDP.Episodic.ae_action_zero_stepLaw, Learning.MDP.Episodic.hasLaw_shiftRound_stepLaw_last, Learning.MDP.Episodic.ae_feedback_eq_zero_stepLaw_last, Learning.MDP.Episodic.ae_feedback_eq_zero_stepLaw, Learning.MDP.Episodic.ae_episodeReturn_eq_add_shiftRound, Learning.MDP.Episodic.ae_stepLaw_castSucc_of_ae, Learning.MDP.Episodic.lintegral_stepLaw_castSucc}\leanok
...
Lemma~\ref{lem:policy_markov} for the layered MDP. For a Lean step \texttt{h : Fin H} (the paper's
$h + 1$), \texttt{hasLaw\_stepLaw\_castSucc} states that under
\texttt{stepLaw M π (s, h.castSucc)} the triple (reward of round $0$, state of round $1$, shifted
trajectory) has law
\texttt{(M.reward h (s, π h s)).prod (M.trans h (s, π h s)) ⊗ₘ Kernel.prodMkLeft ℝ (stepLawKernel M π h.succ)},
which contains (i) and (ii); the almost sure statements and Lebesgue integrals along one step are
\texttt{ae\_stepLaw\_castSucc\_of\_ae} and \texttt{lintegral\_stepLaw\_castSucc}. At the terminal
layer the shifted trajectory has the same law as the trajectory
(\texttt{hasLaw\_shiftRound\_stepLaw\_last}), all rewards vanish almost surely, hence so do the
rewards of the rounds at the terminal layer from any start (\texttt{ae\_feedback\_eq\_zero\_stepLaw}), and
the return is the first reward plus the return of the shifted trajectory almost surely
(\texttt{ae\_episodeReturn\_eq\_add\_shiftRound}): this is the identity
$R^\pi_h = R_h + R^\pi_{h+1}$ used by every backward recursion.
...
\lean{Learning.MDP.Episodic.expValue_castSucc, Learning.MDP.Episodic.expValue_last, Learning.MDP.Episodic.lexpValue_castSucc, Learning.MDP.Episodic.lexpValue_lt_top, Learning.MDP.Episodic.expQ_of_hasRewardFn, Learning.MDP.Episodic.integrable_exp_mul_of_rewardsIn}\leanok
\uses{def:exp_value, lem:step_law_markov}
Assume that the rewards have finite exponential moments of order $\beta$:
$\int e^{\beta x}\,R_h(dx \mid s, a) < \infty$ for all $h, s, a$ (true for bounded rewards, in
particular for a reward function). Then $Z^\pi_{H+1} = 1$ and for every policy $\pi$, step
$h \le H$ and state $s$,
\[
...
with $a = \pi_h(s)$ (\texttt{lexpValue\_castSucc}). The exponential moment hypothesis makes all
$L^\pi_h(s)$ finite by backward induction (\texttt{lexpValue\_lt\_top}), and then
$Z^\pi_h = (L^\pi_h)^{\mathrm{toReal}}$ satisfies the Bellman equation (\texttt{expValue\_castSucc}).
Without it the Bochner-integral identity can fail (a non-integrable exponential return has
Bochner integral $0$), so the hypothesis is not an artifact.
...
\texttt{max 1 (Real.exp (β * (H - h)))} for the Lean state step \texttt{h : Fin (H + 1)}
(\texttt{H - h} steps to go; for \texttt{expQ} at \texttt{h : Fin H} the same exponent). The
...
backward induction with \texttt{ae\_episodeReturn\_eq\_add\_shiftRound} and
\texttt{ae\_stepLaw\_castSucc\_of\_ae}. Positivity \texttt{expValue\_pos} only needs finite
exponential moments.
...
\lean{Learning.MDP.Episodic.expValue_le_optExpValue, Learning.MDP.Episodic.optExpValue_le_expValue, Learning.MDP.Episodic.exists_expValue_eq_optExpValue, Learning.MDP.Episodic.expValue_eq_exp_mul_entropicValue, Learning.MDP.Episodic.entropicValue_le_optEntropicValue, Learning.MDP.Episodic.exists_entropicValue_eq_optEntropicValue}\leanok
\uses{def:opt_value, lem:exp_value_range}
Assume $Z^\pi_h(s) > 0$ for every policy $\pi$ (true for rewards with finite exponential moments,
Lemma~\ref{lem:exp_value_range}, in particular for rewards in $[0, 1]$). If $\beta > 0$ then $Z^*_h(s) = \max_\pi Z^\pi_h(s)$, and if $\beta < 0$
...
\lean{Learning.MDP.Episodic.optPolicy, Learning.MDP.Episodic.expValue_optPolicy, Learning.MDP.Episodic.optExpValue_castSucc_eq, Learning.MDP.Episodic.optExpQ_le_optExpValue, Learning.MDP.Episodic.optExpValue_le_optExpQ, Learning.MDP.Episodic.optExpValue_last, Learning.MDP.Episodic.optExpQ_of_hasRewardFn, Learning.MDP.Episodic.expQ_le_optExpQ, Learning.MDP.Episodic.optExpQ_le_expQ}\leanok
\uses{def:opt_value, lem:exp_bellman, lem:exp_value_monotone, lem:opt_exp_value_eq}
Let $\beta \ne 0$ and assume that the rewards of $\mathcal M$ have finite exponential moments of
order $\beta$ (for instance, $\mathcal M$ has a reward function $r$ with values in $[0, 1]$). Then
...
needed. The formal statements hold at every step \texttt{h : Fin (H + 1)} and only assume
