...
\lean{Essakine2026Tight.bonus_nonneg, Essakine2026Tight.stepU_mem, Essakine2026Tight.optZ_mem, Essakine2026Tight.stepUAt_mem, Essakine2026Tight.optZ_eq, Essakine2026Tight.stepUAt_fst_le_optZ, Essakine2026Tight.optZ_fst_eq_stepUAt_greedy, Essakine2026Tight.optZ_snd_le_stepUAt, Essakine2026Tight.optZ_snd_eq_stepUAt_greedy, Essakine2026Tight.alphaKL_nonneg, Essakine2026Tight.alphaStar_nonneg}\leanok
...
\textbf{Lean remark.} File \texttt{EV2026/Backups.lean}. The one-step statement is
\texttt{stepU\_mem} (for a reward in $[0, 1]$, a probability vector $\phat$ and next-step values
$\Zl \le \Zt$ in the range with \texttt{k} steps to go), and the induction is on the number
\texttt{j ≤ H} of steps to go for \texttt{optZ r β δ hist j} (\texttt{optZ\_mem}); the ranges are
written \texttt{Set.Icc (min 1 (Real.exp (β * j))) (max 1 (Real.exp (β * j)))}, which covers both
signs of $\beta$ at once (the branch \texttt{¬ 0 < β} also covers $\beta = 0$). The bonus
nonnegativity \texttt{bonus\_nonneg} holds unconditionally for $\delta \in (0, 1]$ (the rates are
nonnegative, \texttt{alphaKL\_nonneg}, \texttt{alphaStar\_nonneg}). The identities between the
values and the backups at the greedy action are \texttt{optZ\_eq},
\texttt{optZ\_fst\_eq\_stepUAt\_greedy} and \texttt{stepUAt\_fst\_le\_optZ} ($\beta > 0$),
\texttt{optZ\_snd\_eq\_stepUAt\_greedy} and \texttt{optZ\_snd\_le\_stepUAt} ($\beta \le 0$).
...
\lean{Essakine2026Tight.cert_mem}\leanok
...
\textbf{Lean remark.} \texttt{entropicBPI H r β δ ε s₁ : BPIAlg S A H} (the horizon is an
explicit argument, the reward function \texttt{r : ℕ → S → A → ℝ} being defined on all steps) with
...
\lean{Essakine2026Tight.ae_action_eq_greedy, Essakine2026Tight.stopCond_histAt_stoppingTime, Essakine2026Tight.not_stopCond_histAt_of_lt_stoppingTime, Essakine2026Tight.stoppingTime_le_of_stopCond, Essakine2026Tight.ae_output_eq_greedy}\leanok
...
\textbf{Lean remark.} File \texttt{EV2026/EntropicBPIRun.lean}; the statements hold for runs in
any environment, not only in the episode environment of an MDP. (i) is LML's
\texttt{IsDeterministicAlg.action\_ae\_all\_eq} (or
