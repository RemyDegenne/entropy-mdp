...
environment `statesEnv M s₁` of a finite-horizon MDP `M`, fix a step `h` and a pair `(s, a)`, and
let `ℱ t = σ(history of the first t episodes, policy of the episode t)` be LML's
...
lemma measureReal_statesLaw_stepPair_eq (s₁ : S) (π : Policy S A H) (h : Fin H)
    (s : S) (a : A) :
    (statesLaw M s₁ π).real {τ | (τ h.castSucc, π h (τ h.castSucc)) = (s, a)}
      = occupancy M π s₁ h s a := by
  classical
  rw [statesLaw, map_measureReal_apply measurable_episodeStates
    (Set.to_countable _).measurableSet, occupancy_eq]
  split_ifs with ha
  · congr 1
    ext x
    simp only [Set.mem_preimage, Set.mem_ofPred_eq, episodeStates, Fin.val_castSucc,
      Prod.mk.injEq]
    constructor
    · exact fun hx ↦ hx.1
    · intro hx
      exact ⟨hx, by rw [hx, ha]⟩
  · rw [← measureReal_empty (μ := stepLaw M π (s₁, startStep H))]
    congr 1
    ext x
    simp only [Set.mem_preimage, Set.mem_ofPred_eq, episodeStates, Fin.val_castSucc,
      Prod.mk.injEq, Set.mem_empty_iff_false, iff_false, not_and]
    intro hx
    rw [hx]
    exact Ne.symm ha
...
lemma condExp_visit_filtrationAction_ae_eq [IsFiniteMeasure P] [DecidableEq S] [DecidableEq A]
    (s₁ : S) (hseq : IsAlgEnvSeq O X Y alg (statesEnv M s₁) P) (t : ℕ) (h : Fin H) (s : S)
    (a : A) :
    P[fun ω ↦ if stepPair X Y h t ω = (s, a) then (1 : ℝ) else 0 | hseq.filtrationAction t]
      =ᵐ[P] fun ω ↦ occupancy M (X t ω) s₁ h s a := by
  have hc := hasCondDistrib_feedback_history_action_statesEnv M s₁ hseq t
...
lemma measure_exists_pullCount_lt_le [DecidableEq S] [DecidableEq A] (s₁ : S)
    (hseq : IsAlgEnvSeq O X Y alg (statesEnv M s₁) P) (h : Fin H) (s : S) (a : A) {δ : ℝ}
    (hδ : 0 < δ) :
    P {ω | ∃ t, (pullCount (stepPair X Y h) (s, a) t ω : ℝ)
        < 1 / 2 * ∑ i ∈ range t, occupancy M (X i ω) s₁ h s a - Real.log (1 / δ)}
      ≤ ENNReal.ofReal δ := by
  ...
  have h' := measure_exists_sum_lt_half_mul_sum_sub_le (μ := P) (ℱ := hseq.filtrationAction)
    ...
    (P := fun i ω ↦ occupancy M (X i ω) s₁ h s a)
    (fun i ω ↦ by split_ifs <;> simp)
    ...
    (fun i ω ↦ ⟨occupancy_nonneg M _ s₁ h s a, occupancy_le_one M _ s₁ h s a⟩)
    (fun i ↦ ((measurable_of_countable (fun π : Policy S A H ↦ occupancy M π s₁ h s a)).comp
...
lemma measure_exists_le_pullCount_visitNextState_mem_le [DecidableEq S] [DecidableEq A] (s₁ : S)
    (hseq : IsAlgEnvSeq O X Y alg (statesEnv M s₁) P) (h : Fin H) (s : S) (a : A)
