...
environment `statesEnv M s₁` of a finite-horizon MDP `M` (`X t` is the policy of the episode `t`,
`Y t` its state sequence). Fix a step `h` and a pair `(s, a)`. The pair visited at the step `h`
of the episode `t` is `stepPair X Y h t`, so that the number of visits of `(s, a)` in the first
`t` episodes is LML's `pullCount (stepPair X Y h) (s, a) t` and the episode of the `k`-th visit
(`k ≥ 1`) is `stepsUntil (stepPair X Y h) (s, a) k` (`⊤` if there are fewer than `k` visits): the
"arm" is the pair `(s, a)` at the step `h`. The next state observed at the `k`-th visit is
`visitNextState X Y h s a k`.
...
omit [Fintype S] [Fintype A] [MeasurableSpace S] [MeasurableSingletonClass S] [MeasurableSpace A]
  [MeasurableSingletonClass A] [Nonempty A] [IsFiniteMeasure P] in
variable (X Y) in
...
def infoBeforeStep (t : ℕ) (h : Fin H) (ω : Ω) :
  ...
  (((history O X Y t ω, O t ω), X t ω), fun i ↦ Y t ω (Fin.castLE (castLE_succ_le h) i))

omit [Fintype S] [Fintype A] [MeasurableSingletonClass S] [MeasurableSingletonClass A] [Nonempty A]
...
lemma measure_inter_feedback_succ_eq_mul (s₁ : S)
    (hseq : IsAlgEnvSeq O X Y alg (statesEnv M s₁) P) (t : ℕ) (h : Fin H) (s : S) (a : A)
  ...
  have hc : HasCondDistrib (fun ω ↦ Y t ω h.succ) (infoBeforeStep O X Y t h)
      (episodeStepKernel M (Hist Unit (Policy S A H) (Traj S H) t × Unit) h) P :=
    hasCondDistrib_feedback_succ_statesEnv M s₁ hseq t h
  ...
  have h2 := (hB q hq).2
  rw [h1] at h2
  simp only [episodeStepKernel, Kernel.comap_apply, h1, h2]
...
lemma integral_mul_feedback_succ_statesEnv (s₁ : S)
    ...
    ∫ ω, Ψ (infoBeforeStep O X Y t h ω) * f (Y t ω h.succ) ∂P
      = ∫ ω, Ψ (infoBeforeStep O X Y t h ω)
          * vecExp (M.transVec h (Y t ω h.castSucc) (X t ω h (Y t ω h.castSucc))) f ∂P := by
  have hc : HasCondDistrib (fun ω ↦ Y t ω h.succ) (infoBeforeStep O X Y t h)
      (episodeStepKernel M (Hist Unit (Policy S A H) (Traj S H) t × Unit) h) P :=
    hasCondDistrib_feedback_succ_statesEnv M s₁ hseq t h
  ...
  simp only
  rw [integral_const_mul, episodeStepKernel, Kernel.comap_apply, integral_eq_vecExp_transVec]
  rfl
...
lemma stepPair_eq_of_infoBeforeStep_eq {t : ℕ} {h : Fin H} {ω ω' : Ω}
  ...
  · have := congrFun hY (Fin.last h)
    rw [show Fin.castLE (castLE_succ_le h) (Fin.last h) = h.castSucc from Fin.ext rfl] at this
...
lemma measure_inter_visitNextState_eq_mul (s₁ : S)
    (hseq : IsAlgEnvSeq O X Y alg (statesEnv M s₁) P) (h : Fin H) (s : S) (a : A) {k : ℕ}
    (hk : k ≠ 0) {E : Set Ω}
  ...
  have hB (t : ℕ) : ∀ q ∈ infoBeforeStep O X Y t h '' F t,
    ...
    have e : Fin.castLE (castLE_succ_le h) (Fin.last h) = h.castSucc := Fin.ext rfl
...
omit [Fintype S] [Fintype A] [MeasurableSpace S] [MeasurableSingletonClass S] [MeasurableSpace A]
  [MeasurableSingletonClass A] [Nonempty A] [IsFiniteMeasure P] [DecidableEq S] [DecidableEq A] in
...
lemma measure_visitNextState_succ_eq_mul (s₁ : S)
    (hseq : IsAlgEnvSeq O X Y alg (statesEnv M s₁) P) (h : Fin H) (s : S) (a : A) (n : ℕ)
    (w : Fin (n + 1) → S) :
