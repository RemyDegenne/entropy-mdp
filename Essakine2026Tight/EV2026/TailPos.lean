...
lemma cert_le_exp_mul_sum_integral_of_pos (hM : M.HasRewardFn r)
  ...
  have hu : ∀ (h : Fin H) (s : S) (a : A), 0 ≤ 36 * √(vecVar (M.transVec h s a)
        (expValue M β (greedy r β δ hist) h.succ)
    ...
    fun h s a ↦ add_nonneg (mul_nonneg (by norm_num) (Real.sqrt_nonneg _))
      (mul_nonneg (by positivity) (rateMin_nonneg (alphaKL_nonneg _ _ _ hδ hδ1) _))
  ...
  have hg : ∀ (h : Fin H) (s : S), cert r β δ hist (H - (h.castSucc : ℕ)) s
    ...
    intro h s
    have := cert_le_of_pos hM hr hβ hδ hδ1 hc h s
    rw [Fin.val_castSucc, sub_succ_eq]
    exact this.trans_eq (by ring)
  exact le_exp_mul_sum_integral_of_backward (M := M) (π := greedy r β δ hist) (s₁ := s₁)
...
lemma cert_div_optZ_fst_le_of_pos (hM : M.HasRewardFn r)
  ...
  have hR := integrable_exp_mul_of_rewardsIn M β hRw
  -- the denominators
  have hZ1 : 1 ≤ expValue M β π (startStep H) s₁ :=
    (expValue_mem_Icc_of_pos hM hr hβ π (startStep H) s₁).1
  have hZle : expValue M β π (startStep H) s₁ ≤ (optZ r β δ hist H).1 s₁ :=
    (expValue_le_optExpValue M β hβ (by linarith)).trans
      (optExpValue_mem_Icc_optZ_of_pos hM hr hβ hδ hδ1 hc (startStep H) s₁).2
  ...
  have hsplit : ∑ h : Fin H,
      ...
      = 36 * ∑ h : Fin H, ∫ x, Real.exp (β * ∑ i ∈ Iic h, r i (IT.obs i x).1 (IT.action i x))
        ...
        + ∑ h : Fin H, ∫ x, Real.exp (β * ∑ i ∈ Iic h, r i (IT.obs i x).1 (IT.action i x))
          * ((fun (h : Fin H) (_ : S) (_ : A) ↦ 84 * H * Real.exp (β * (H - h : ℕ)))
    ...
    · exact (integrable_exp_sum_mul P β r (Iic h)
        (fun s a ↦ √(vecVar (M.transVec h s a) (expValue M β π h.succ) * ρs h s a)) h).const_mul 36
    · exact integrable_exp_sum_mul P β r (Iic h)
        (fun s a ↦ 84 * H * Real.exp (β * (H - h : ℕ)) * ρ h s a) h
  have hvar := sum_integral_exp_mul_sqrt_vecVar_mul_le (π := π) (s₁ := s₁) hM β hρs0
  have hrate := sum_integral_exp_mul_mul_le (M := M) (π := π) (s₁ := s₁) (r := r) β hρ0
    (c := fun (h : Fin H) (_ : S) (_ : A) ↦ 84 * H * Real.exp (β * (H - h : ℕ)))
    (C := 84 * H * Real.exp (|β| * (H + 1 : ℕ))) fun h x ↦ by
      have hsum : ∑ i ∈ Iic h, r i (IT.obs i x).1 (IT.action i x) ≤ (h : ℝ) + 1 :=
        calc ∑ i ∈ Iic h, r i (IT.obs i x).1 (IT.action i x) ≤ ∑ _i ∈ Iic h, (1 : ℝ) :=
              sum_le_sum fun i _ ↦ (hr _ _ _).2
          _ = (h : ℝ) + 1 := by simp [Fin.card_Iic]
      have hcast : ((H - h : ℕ) : ℝ) = H - h := Nat.cast_sub h.is_lt.le
  ...
  have hZpos : 0 < expValue M β π (startStep H) s₁ := by linarith
  ...
  calc cert r β δ hist H s₁
    ...
    _ ≤ Real.exp 13 * (36 * ((√(varianceFactor β (maxReturn M s₁))
        ...
        + 84 * H * Real.exp (|β| * (H + 1 : ℕ))
          ...
          * expValue M β π (startStep H) s₁) := by
    ...
    certAt X Y r β δ t ω (startStep H) s₁ ≤ Real.exp 13 * ∑ h : Fin H,
      ∫ x, Real.exp (β * ∑ i ∈ Iic h, r i (IT.obs i x).1 (IT.action i x))
        * (36 * √(vecVar (M.transVec h (IT.obs h x).1 (IT.action h x))
          ...
          + 84 * H * Real.exp (β * (H - h : ℕ))
            * klRateMinAt X Y δ h (IT.obs h x).1 (IT.action h x) t ω)
...
lemma certAt_div_ZtildeAt_le_of_pos (hM : M.HasRewardFn r)
    (hr : ∀ h s a, r h s a ∈ Set.Icc 0 1) (hβ : 0 < β) (hδ : 0 < δ) (hδ1 : δ ≤ 1)
    (hω : ω ∈ goodEvent X Y M s₁ β δ) (t : ℕ) :
    certAt X Y r β δ t ω (startStep H) s₁ / ZtildeAt X Y r β δ t ω (startStep H) s₁
