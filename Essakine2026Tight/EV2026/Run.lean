...
noncomputable def ZtildeAt (r : Fin H → S → A → ℝ) (β δ : ℝ) (t : ℕ) (ω : Ω) (h : Fin (H + 1)) :
    S → ℝ :=
  (optZ r β δ (histAt X Y t ω) (H - h)).1

/-- The pessimistic exponential value `Z̲_h^t` at episode `t`. -/
noncomputable def ZlowerAt (r : Fin H → S → A → ℝ) (β δ : ℝ) (t : ℕ) (ω : Ω) (h : Fin (H + 1)) :
    S → ℝ :=
  (optZ r β δ (histAt X Y t ω) (H - h)).2

/-- The optimistic backup `Ũ_h^t(s, a)` at episode `t`. -/
noncomputable def UtildeAt (r : Fin H → S → A → ℝ) (β δ : ℝ) (t : ℕ) (ω : Ω) (h : Fin H) (s : S)
    (a : A) : ℝ :=
  (stepUAt r β δ (histAt X Y t ω) h s a).1

/-- The pessimistic backup `U̲_h^t(s, a)` at episode `t`. -/
noncomputable def UlowerAt (r : Fin H → S → A → ℝ) (β δ : ℝ) (t : ℕ) (ω : Ω) (h : Fin H) (s : S)
    (a : A) : ℝ :=
  (stepUAt r β δ (histAt X Y t ω) h s a).2

/-- The bonus `b_h^t(s, a)` at episode `t`. -/
noncomputable def bonusAt (r : Fin H → S → A → ℝ) (β δ : ℝ) (t : ℕ) (ω : Ω) (h : Fin H) (s : S)
    (a : A) : ℝ :=
...
noncomputable def greedyAt (r : Fin H → S → A → ℝ) (β δ : ℝ) (t : ℕ) (ω : Ω) : Policy S A H :=
  greedy r β δ (histAt X Y t ω)

/-- The certificate `π^{t+1} G_h^t` at episode `t`. -/
noncomputable def certAt (r : Fin H → S → A → ℝ) (β δ : ℝ) (t : ℕ) (ω : Ω) (h : Fin (H + 1)) :
    S → ℝ :=
  cert r β δ (histAt X Y t ω) (H - h)

/-- The auxiliary value `Z̊_h^t` at episode `t`. -/
noncomputable def ringZAt (M : EpisodicMDP S A H) (r : Fin H → S → A → ℝ) (β δ : ℝ) (t : ℕ)
    (ω : Ω) (h : Fin (H + 1)) : S → ℝ :=
  ringZ M r β δ (histAt X Y t ω) (H - h)

/-- The auxiliary backup `Ů_h^t(s, a)` at episode `t`. -/
noncomputable def ringUAt (M : EpisodicMDP S A H) (r : Fin H → S → A → ℝ) (β δ : ℝ) (t : ℕ)
...
noncomputable def pseudoCount (M : EpisodicMDP S A H) (s₁ : S) (h : Fin H) (s : S) (a : A)
    (t : ℕ) (ω : Ω) : ℝ :=
  ∑ i ∈ range t, occupancy M (X i ω) s₁ h s a
...
def eventKL (M : EpisodicMDP S A H) (δ : ℝ) : Set Ω :=
...
def eventBern (M : EpisodicMDP S A H) (δ : ℝ) (f : Fin (H + 1) → S → ℝ) (b : Fin H → ℝ) :
    Set Ω :=
  {ω | ∀ t h s a, 0 < visitCountAt X Y h s a t ω →
    |vecExp (empTransAt X Y h s a t ω) (f h.succ) - vecExp (M.transVec h s a) (f h.succ)| ≤
      √(2 * vecVar (M.transVec h s a) (f h.succ) * starRateAt X Y δ h s a t ω)
      + 3 * b h * starRateAt X Y δ h s a t ω}

/-- The counts concentration event `𝓔^cnt`: `n_h^t(s, a) ≥ n̄_h^t(s, a) / 2 - α^cnt(δ)`. -/
def eventCnt (M : EpisodicMDP S A H) (s₁ : S) (δ : ℝ) : Set Ω :=
...
def goodEvent (M : EpisodicMDP S A H) (s₁ : S) (β δ : ℝ) : Set Ω :=
  eventKL X Y M δ ∩ eventBern X Y M δ (optExpValue M β)
