...
variable {S A : Type*} [Fintype S] [Fintype A] [DecidableEq S] [DecidableEq A] [Nonempty A]
  ...
  {r : ℕ → S → A → ℝ} {β δ ε : ℝ} {s₁ : S} {env : Environment Unit (Policy S A H) (Traj S H)}
...
lemma ae_action_eq_greedy
    (h : (entropicBPI H r β δ ε s₁).IsRun env (fun _ _ ↦ ()) X Y out P) :
...
lemma stopCond_histAt_stoppingTime {ω : Ω}
    (hτ : (entropicBPI H r β δ ε s₁).stoppingTime (fun _ _ ↦ ()) X Y ω ≠ ⊤) :
    stopCond r β δ ε s₁
      (histAt X Y ((entropicBPI H r β δ ε s₁).stoppingTime (fun _ _ ↦ ()) X Y ω).untopA ω) :=
...
lemma not_stopCond_histAt_of_lt_stoppingTime {ω : Ω} {t : ℕ}
    (ht : (t : ℕ∞) < (entropicBPI H r β δ ε s₁).stoppingTime (fun _ _ ↦ ()) X Y ω) :
...
lemma stoppingTime_le_of_stopCond {ω : Ω} {t : ℕ} (ht : stopCond r β δ ε s₁ (histAt X Y t ω)) :
    (entropicBPI H r β δ ε s₁).stoppingTime (fun _ _ ↦ ()) X Y ω ≤ t :=
...
lemma ae_output_eq_greedy
    ...
    ∀ᵐ ω ∂P, out ω = greedy r β δ
      (histAt X Y ((entropicBPI H r β δ ε s₁).stoppingTime (fun _ _ ↦ ()) X Y ω).untopA ω) := by
  have hout : HasCondDistrib out ((entropicBPI H r β δ ε s₁).stoppedHist (fun _ _ ↦ ()) X Y)
